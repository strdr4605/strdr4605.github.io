...
npm() {
  ...
  then
    ...
    # Run 'which npm' to get your specific path
    /usr/local/bin/npm $@
  fi
}
```

If this function is not working outside meteor projects, you may have a different path to `npm` bin file. Replace `/usr/local/bin/npm` in the script.
